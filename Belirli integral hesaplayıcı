...
{
    public class Calculatorhpi
{
    private string expression;
    private int position;

    public Calculatorhpi(string expression)
    {
...
public class Programnormall
{
    public static string normalhesapsonucc(string expression)
    {
        Calculatorhpi calculator = new Calculatorhpi(expression);
    ...
    public static double belirliintegralsonuc(string input,double a,double b)
    {
        Console.WriteLine("Hesaplama işlemi için bir ifade girin:");
        
        double n=100000;
        if (n % 2 != 0) n++; // Adım sayısı tekse, onu çift yap

        // Adım büyüklüğünü hesapla
        double h = (b - a) / n;
    
        // Başlangıç ve bitiş noktalarındaki değerleri hesapla
        double sum = Convert.ToDouble(normalhesapsonucc(input.Replace("x", a.ToString()))) +Convert.ToDouble(normalhesapsonucc(input.Replace("x", b.ToString())));

        // Tek noktalar için katsayı 4 ile çarpılır
        for (int i = 1; i < n; i += 2)
        {
            double x = a + i * h;
            sum += 4 * Convert.ToDouble(normalhesapsonucc(input.Replace("x", x.ToString())));
        }

        // Çift noktalar için katsayı 2 ile çarpılır
        for (int i = 2; i < n - 1; i += 2)
       {
            double x = a + i * h;
            sum += 2 * Convert.ToDouble(normalhesapsonucc(input.Replace("x", x.ToString())));
        }

        // Simpson formülünü uygula
        return sum * h / 3;
    }
}
